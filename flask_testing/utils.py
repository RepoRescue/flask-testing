...
class TestCase(unittest.TestCase):
    ...
    def assertTemplateUsed(self, name, tmpl_name_attribute='name'):
        ...
        raise AssertionError("Template %s not used. Templates were used: %s" % (name, repr(used_templates)))
    ...
    def assertRedirects(self, response, location, message=None):
        ...
        # Normalize response.location to absolute URL if it's relative
        actual_location = response.location
        if actual_location and not urlparse(actual_location).netloc:
            server_name = self.app.config.get('SERVER_NAME') or 'localhost'
            actual_location = urljoin("http://%s" % server_name, actual_location)
